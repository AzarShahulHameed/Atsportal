...
export const DEFAULT_TEMPLATES: Record<StatusKey, { subject: string; bodyHtml: string }> = {
  ...
  INTERVIEW_SCHEDULED: {
    ...
    bodyHtml: `<p>Hi {{candidateName}},</p>
      <p>We'd like to move forward with an interview for the <strong>{{jobTitle}}</strong> role. Someone from our team will contact you separately with timing and format.</p>`,
  },
  OFFERED: {
...
export const AVAILABLE_PLACEHOLDERS = ['{{candidateName}}', '{{jobTitle}}', '{{companyName}}'] as const;
...
// looks like it came from the same company as the default ones.
export function wrapEmailBody(companyName: string, bodyHtml: string): string {
  return `
